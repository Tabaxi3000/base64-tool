...
from base64_tool.encoders import encode_ascii85, encode_rot13
...
class TestDetectRot13:
    def test_rot13_english_detected(self) -> None:
        """
        Checks that ROT13-encoded English prose is detected as ROT13
        """
        encoded = encode_rot13(
            b"the quick brown fox jumps over the lazy dog"
        )
        result = detect_best(encoded)
        assert result is not None
        assert result.format == EncodingFormat.ROT13
        assert result.confidence >= 0.6

    def test_plain_english_not_rot13(self) -> None:
        """
        Checks that ordinary English text is not flagged as ROT13
        """
        results = detect_encoding("the quick brown fox jumps over")
        assert all(r.format != EncodingFormat.ROT13 for r in results)

    def test_base64_not_scored_as_rot13(self) -> None:
        """
        Checks that a base64 string is never detected as ROT13
        """
        results = detect_encoding("SGVsbG8gV29ybGQ=")
        assert all(r.format != EncodingFormat.ROT13 for r in results)


class TestDetectAscii85:
    def test_ascii85_detected(self) -> None:
        """
        Checks that ASCII85-encoded text is detected as ASCII85
        """
        encoded = encode_ascii85(b"Hello World from ascii85 encoding")
        result = detect_best(encoded)
        assert result is not None
        assert result.format == EncodingFormat.ASCII85
        assert result.confidence >= 0.6

    def test_adobe_wrapped_detected(self) -> None:
        """
        Checks that Adobe <~...~> wrapped ASCII85 is detected with high confidence
        """
        encoded = "<~" + encode_ascii85(b"wrapped payload") + "~>"
        result = detect_best(encoded)
        assert result is not None
        assert result.format == EncodingFormat.ASCII85
        assert result.confidence >= 0.8

    def test_base64_not_scored_as_ascii85(self) -> None:
        """
        Checks that a padded base64 string is not detected as ASCII85
        """
        results = detect_encoding("SGVsbG8gV29ybGQ=")
        assert all(r.format != EncodingFormat.ASCII85 for r in results)
