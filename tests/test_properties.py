...
class TestRot13Properties:
    ...
    def test_roundtrip(self, data: bytes) -> None:
        """
        Checks that arbitrary binary data survives a ROT13 roundtrip unchanged
        """
        assert decode_rot13(encode_rot13(data)) == data

    @given(st.binary())
    def test_double_application_is_identity(self, data: bytes) -> None:
        """
        Checks that applying ROT13 twice returns the original data
        """
        once = encode_rot13(data)
        twice = encode_rot13(once.encode("latin-1"))
        assert twice.encode("latin-1") == data


class TestAscii85Properties:
    @given(st.binary())
    def test_roundtrip(self, data: bytes) -> None:
        """
        Checks that arbitrary binary data survives an ASCII85 roundtrip unchanged
        """
    ...
    def test_encoded_is_ascii(self, data: bytes) -> None:
        """
        Checks that ASCII85 output is always valid ASCII
        """
        encode_ascii85(data).encode("ascii")


class TestCustomAlphabetProperties:
    @given(st.binary())
    def test_roundtrip(self, data: bytes) -> None:
        """
        Checks that a scrambled base64 alphabet round-trips arbitrary binary data
        """
        encoded = encode_base64_custom(data, _REVERSED_ALPHABET)
        assert decode_base64_custom(encoded, _REVERSED_ALPHABET) == data
...
class TestCrossFormatProperties:
    ...
    def test_all_formats_roundtrip(self, data: bytes) -> None:
        ...
        for fmt in (
            ...
            EncodingFormat.HEX,
            EncodingFormat.ROT13,
            EncodingFormat.ASCII85,
        ):
