...
def _score_rot13(data: str) -> float:
    ...
    # Only consider inputs that already look like prose. Base64/base32/hex
    # payloads carry '=', '+', '/' or are digit-heavy, so excluding them keeps
    # ROT13 from competing with the structured-encoding scorers.
    if any(c in stripped for c in "=+/"):
        return 0.0
    textlike = sum(
        1 for c in stripped if c.isalpha() or c in _ROT13_TEXT_CHARS
    )
    if textlike / len(stripped) < 0.85:
        return 0.0

    decoded = try_decode(stripped, EncodingFormat.ROT13)
    if decoded is None:
        return 0.0
    decoded_text = decoded.decode("latin-1")

    source_score = english_letter_score(stripped)
    decoded_score = english_letter_score(decoded_text)
    if decoded_score <= source_score:
        return 0.0
    if decoded_score < ROT13_ENGLISH_FLOOR:
        return 0.0

    gain = decoded_score - source_score
    score = W.ROT13_BASE + min(
        gain * W.ROT13_ENGLISH_GAIN_MULTIPLIER,
        W.ROT13_ENGLISH_GAIN_CAP,
    )
    if common_word_ratio(decoded_text) >= 0.3:
        score += W.ROT13_COMMON_WORDS
...
def _score_ascii85(data: str) -> float:
    stripped = "".join(data.split())
    wrapped = stripped.startswith(ASCII85_ADOBE_PREFIX) and stripped.endswith(
        ASCII85_ADOBE_SUFFIX
    )
    core = stripped[2: -2] if wrapped else stripped

    if len(core) < MIN_INPUT_LENGTH:
        return 0.0
    if not all(c in ASCII85_CHARSET for c in core):
        return 0.0
    # A final ASCII85 group is at least two characters, so a length of the form
    # 5k+1 can never be valid.
    if len(core) % 5 == 1:
        return 0.0
    ...
    if wrapped:
        score += W.A85_WRAPPER
    if any(c in ASCII85_SPECIAL_CHARS for c in core):
        score += W.A85_SPECIAL_CHARS
    score += W.DECODE_SUCCESS

    if is_printable_text(decoded):
        score += W.PRINTABLE_RESULT
    elif not wrapped:
        # Unwrapped ASCII85 shares its character set with plain text and other
        # encodings, so require a readable decode to avoid false positives.
        return 0.0
